...
public class AutoGemMinerOverlay extends OverlayPanel {
    private final AutoGemMiner plugin;

    @Inject
    public AutoGemMinerOverlay(AutoGemMiner plugin) {
        this.plugin = plugin;
    ...
    private void addFullOverlay() {
        ...
        int totalSupplies = plugin.getGemCounter();
        ...
        String suppliesGathered = String.format("%d (%d/hr)", totalSupplies, suppliesPerHour);
        panelComponent.getChildren().add(LineComponent.builder()
                .left("Gems Mined:")
                .right(suppliesGathered)
                .leftColor(Color.WHITE)
                .build());
